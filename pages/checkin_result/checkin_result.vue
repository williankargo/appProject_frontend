<template>
	<view class="page">
		<view class="summary-container">
			<view class="user-info">
				...
				<view class="info">
					...
					<text class="dept">隸屬部門：{{deptName == null ? "" : deptName}}</text>
				</view>
			</view>
		...
		<view class="result-container">
			...
			<view class="right">
				<view class="row">
					<text class="start">上班(時間{{attendanceTime}})</text>
				</view>
				<view class="row">
					<text class="checkin-time">簽到時間（{{checkinTime}}）</text>
					<text class="checkin-result green other" v-if="status=='正常'">{{status}}</text>
					<text class="checkin-result yellow other" v-if="status=='遲到'">{{status}}</text>
				</view>
				<view class="row">
					<image src="../../static/icon-7.png" mode="widthFix" class="icon-small"></image>
					<text class="desc">{{address}}</text>
					<text class="checkin-result green" v-if="risk=='低風險'">{{risk}}</text>
					<text class="checkin-result yellow" v-if="risk=='中風險'">{{risk}}</text>
					<text class="checkin-result red" v-if="risk=='高風險'">{{risk}}</text>
					
				</view>
				<view class="row">
					<image src="../../static/icon-8.png" mode="widthFix" class="icon-small"></image>
					<text class="desc">身份驗證</text>
					<text class="checkin-result green">已通過</text>
				</view>
				<view class="row">
					<text class="end">下班(時間{{closingTime}})</text>
				</view>
			</view>
		</view>
		<view class="checkin-report">
			...
			<view class="calendar-container">
				<view class="calendar" v-for="one in weekCheckin" :key="one"> <!-- 定義key 避免重複產生DOM元素而浪費資源 -->
					...
					<text class="result yellow" v-if="one.status=='遲到'">{{one.status}}</text>
